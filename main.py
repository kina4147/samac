...
MotionModel = namedtuple('MotionModel', ['dim_x', 'dim_z', 'model'])
@jit
def poly_area(x, y):
...
def associate_detections_to_trackers(dets=None, trks=None, trks_S=None,
...
class AB3DMOT(object):
    ...
    def update(self, dets_all, match_distance, match_threshold, match_algorithm, seq_name):
        ...
        for t in range(len(self.trackers)):
            pred_z, S = self.trackers[t].predict()
            if np.any(np.isnan(pred_z)): # ERROR TRACK REMOVAL
                to_del.append(t)
            else:
        ...
        # DATA ASSOCIATION
        matched, unmatched_dets, unmatched_trks = associate_detections_to_trackers(dets=dets, trks=trks, trks_S=trks_S,
        ...
        for t, trk in enumerate(self.trackers):
            if t not in unmatched_trks:
                ...
                trk.track_score = info[d, :][0][-1] # detection_score

        # NEW TRACK GENERATION
        # create and initialise new trackers for unmatched detections
        for i in unmatched_dets:  # a scalar of index
            # detection_score = info[i][-1]
            track_score = info[i][-1] # detection_score
            model = MotionModel(11, 7, 'cv')
            trk = KFTracker(dets[i, :], info[i, :], model, track_score, self.tracking_name)
    ...
    for sample_token_idx in tqdm(range(len(all_results.sample_tokens))):
        ...
        while current_sample_token != '':
            ...
            for box in all_results.boxes[current_sample_token]:
                ...
                dets[box.detection_name].append(detection) # detection state
                info[box.detection_name].append(information) # detection score
